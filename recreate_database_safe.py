#!/usr/bin/env python3
"""
Safe script to drop all existing tables and recreate the database schema from scratch.
This version asks for confirmation before dropping any data.
"""
import asyncio
import asyncpg
import os
...
async def recreate_database_safe():
    """Drop all existing tables and recreate with new schema (with confirmation)."""
    ...
    try:
        ...
        if tables:
            ...
            for table in tables:
                ...
                count_result = await conn.fetch(f'SELECT COUNT(*) as count FROM "{table_name}"')
            ...
            if tables_with_data:
                ...
                print("This operation will PERMANENTLY DELETE all data in these tables:")
                for table_name, count in tables_with_data:
                    print(f"  - {table_name}: {count} rows will be lost")
                
                print("\n" + "="*60)
                print("🚨 DANGER: This will permanently delete all data!")
                print("="*60)
...
if __name__ == "__main__":
    asyncio.run(recreate_database_safe())
